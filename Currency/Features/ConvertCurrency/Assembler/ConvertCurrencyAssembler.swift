...
extension DependencyAssemblerManager: ConvertCurrencyFactory {
    ...
    func makeConvertCurrencyVM(coordinator: ConvertCurrencyCoordinator) -> ConvertCurrencyVM {
        let viewModel = ConvertCurrencyVM(repository: convertCurrencyRepository)
        return viewModel
    }
    
    func makeConvertCurrencyVC(coordinator: ConvertCurrencyCoordinator) -> ConvertCurrencyVC? {

        if let viewController = UIStoryboard(name: AppConstants.mainStoryBoard, bundle: nil).instantiateViewController(withIdentifier: AppConstants.StoryBoardIds.ConvertCurrency.rawValue) as? ConvertCurrencyVC {
